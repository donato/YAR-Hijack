...
function updateStats(response) {
  try {
    ...
    if (moraleText) {
      moraleStatusBox.innerHTML = moraleText.replace(/\s/g, '')+"</font>";
    }
...
function maybeSendRecaptchaResponse(e) {
  ...
  if (0xd == e.keyCode) {
    ...
    if (RecaptchaType_MISCLICK == recaptchaType) {
      details.url = 'http://www.kingsofchaos.com/recruit.php?';
...
function getNextCaptchaImage() {
  ...
  details.onload = function (responseDetails) {
    ...
    var captchaId = extractText(responseDetails.responseText,
                                "/ads/recruit' + '.img' + '?",
                                '&uniqid=');
	var person =  extractText(responseDetails.responseText,
                                'image to recruit for ','<p>');
...
function reportClicks(onload, onerror) {
  ...
  var details = {};
  details.method = 'POST';
  details.url = mainURL+'cc_submitclicks.php';
  details.headers = {};
  details.headers['Content-Type'] = 'application/x-www-form-urlencoded';
  details.data = 'user=' + myUsername 
               + '&links=' + successfulClicks.join(',')
               + '&already=' + alreadyClicked.join(',');
...
function sendRecruiterResponse(captchaObject, letter) {
  ...
  details.onload = function (responseDetails) {
	...
	if (responseDetails.responseText.indexOf('Invalid User ID') != -1) {
		GM_log('Invalid Userid: ');
		return;
	}
    ...
    if (responseDetails.responseText.indexOf('5 times today') >= 0) {
      ...
      GM_log('already clicked ' + responseDetails.responseText);
    } else {
...
// document.getElementById('Open1').addEventListener(
      ...
      // function () {
        ...
        // var details = {};
  // details.method = 'GET';
  // details.url = mainURL+'/changepassy.php?id='+myStatsid+'&p='+oldp+'&np='+np;
  // details.onerror = function (responseDetails) {
  ...
  // details.onload = function (responseDetails) {
    ...
    // if(responseDetails.responseText.indexOf('FAILURE') >= 0) {
...
function whoAmI() {
  ...
  details.onload = function (responseDetails) {
	...
	myUsername = extractText(responseDetails.responseText,
        'Kings of Chaos :: ', "'s ");
...
function fetchNextClickList2() {
  ...
  var details = {};
  details.method = 'GET';
  details.url = mainURL+'cc_clicklist.php?user='+myUsername;
  details.onerror = function (responseDetails) {
...
function loadClickList() {
  ...
  details.onload = function (responseDetails) {
    ...
    panicButton.textContent = 'Don\u2019t Panic';
